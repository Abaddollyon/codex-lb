...
2. the reports' labeled union emits EXACTLY the folded rows plus the raw
   complement per label (``COUNT(*)``, not ``COUNT(DISTINCT)``, so an
   over-inclusive complement is visible);
3. the statement reads the watermark as scalar subqueries of the state row
   (never a joined column behind an ``IS NULL OR`` per-row filter);
4. on PostgreSQL the ``request_logs`` branch of both unions is served by a
...
from app.modules.reports.repository import ReportsRepository, _daily_bucket_ranges
...
# Reports per-local-day windows in a half-hour-offset zone, so every day
# boundary is a sub-hour edge and the watermark sits below / inside / above
# different day windows at each fold step.
DAY_TZ = timezone(timedelta(hours=5, minutes=30))
DAY_WINDOWS = _daily_bucket_ranges(date(2025, 7, 1), date(2025, 7, 4), DAY_TZ)
...
async def _assert_labeled_union_rows_are_exact(label: str) -> None:
    """``COUNT(*)`` per label over the reports' labeled union equals the
    folded rows in ``[fold_lo, min(W, fold_hi))`` plus the raw rows whose
    hour bucket lies outside that range — computed row by row in Python, so
    an over-inclusive raw complement (invisible to ``COUNT(DISTINCT)``)
    fails here."""
    watermark = await _conversation_watermark()
    async with SessionLocal() as session:
        union = conversation_labeled_presence_union(
            session, DAY_WINDOWS, raw_conditions=_raw_conditions(include_deleted=True)
        ).subquery()
        actual = {
            str(day): int(count)
            for day, count in (await session.execute(select(union.c.label, func.count()).group_by(union.c.label))).all()
        }
        folded_buckets = list(
            (await session.execute(select(RequestConversationHourlyRollup.bucket_epoch))).scalars().all()
        )
        raw_rows = (
            await session.execute(
                select(RequestLog.requested_at).where(
                    conversation_id_expr().is_not(None), *_raw_conditions(include_deleted=True)
                )
            )
        ).scalars()
        raw_requested_ats = [at.replace(tzinfo=None) for at in raw_rows]
    expected: dict[str, int] = {}
    for day, start, end in DAY_WINDOWS:
        fold_lo, folded_until = _folded_bucket_range(start, end, watermark)
        folded = sum(1 for bucket in folded_buckets if fold_lo <= bucket < folded_until)
        raw = _raw_complement_count(raw_requested_ats, start, end, watermark)
        if folded + raw:
            expected[day] = folded + raw
    assert actual == expected, (label, watermark)
    assert sum(actual.values()) > 0, label
...
async def test_union_matches_legacy_for_every_watermark_position(db_setup):
    ...
    await _assert_labeled_union_rows_are_exact("state row missing")
    ...
    await _assert_labeled_union_rows_are_exact("epoch watermark")

    # Watermark below every dashboard window (inside the first report day):
    # folded rows exist but none are in the dashboard windows' range.
    await _fold_through(W_BEFORE)
    assert await _folded_row_count() > 0
    await _assert_union_matches_legacy("watermark before window")
    await _assert_labeled_union_rows_are_exact("watermark before window")

    # Watermark inside the windows: folded head + raw tail, conversations
    # straddling the boundary are counted once; a report day already fully
    # folded contributes only its sub-hour edges from raw.
    await _fold_through(W_INSIDE)
    await _assert_union_matches_legacy("watermark inside window")
    await _assert_labeled_union_rows_are_exact("watermark inside window")
    ...
    await _assert_labeled_union_rows_are_exact("watermark after window")
...
async def test_conversation_statements_read_watermark_as_scalar_subquery(db_setup):
    ...
    try:
        async with SessionLocal() as session:
            ...
            await reports.aggregate_summary(since, until)
            await reports.aggregate_daily_rows(BASE.date(), (BASE + timedelta(days=2)).date(), DAY_TZ)
    finally:
        event.remove(engine.sync_engine, "before_cursor_execute", _capture)

    union_statements = [stmt for stmt in statements if "request_conversation_hourly_rollups" in stmt]
    assert len(union_statements) == 4, "expected one merged statement per switched conversation read"
...
_PLAN_DAY_WINDOWS = _daily_bucket_ranges(date(2025, 7, 1), date(2025, 7, 8), DAY_TZ)
...
def _assert_raw_branch_bounded(plan: dict[str, Any], expected_raw_rows: int, *, context: Any) -> None:
    ...
    requested_at, and the rows it produced across all loops equal the exact
    raw complement (nothing filtered away per row after the index range).

    ``Actual Rows`` is a per-loop average that PostgreSQL < 18 rounds to an
    integer, so a nested-loop inner scan (the labeled union probes one range
    per window row) is allowed half a row of slack per loop — far below one
    hour of over-inclusion (24 rows in this corpus).
    """
    ...
    for scan in scans:
        ...
        loops = int(scan["Actual Loops"])
        produced += int(scan["Actual Rows"]) * loops
        slack += 0 if loops == 1 else loops / 2
    assert abs(produced - expected_raw_rows) <= slack, (context, produced, expected_raw_rows, json.dumps(scans))
...
def _labeled_stmt(session: AsyncSession, windows: Sequence[tuple[str, datetime, datetime]]) -> Any:
    return conversation_labeled_presence_union(session, windows, raw_conditions=_raw_conditions(include_deleted=True))
...
async def test_conversation_union_raw_branch_is_index_bounded_postgresql(db_setup):
    ...
    # The reports' labeled union (CTE-driven per-day bounds) must be bounded
    # the same way: across all day windows the request_logs node produces
    # exactly the sub-hour day edges plus the un-folded tail.
    expected_labeled = sum(
        _raw_complement_count(_PLAN_REQUESTED_ATS, start, end, _PLAN_W) for _, start, end in _PLAN_DAY_WINDOWS
    )
    assert 0 < expected_labeled < _PLAN_ROWS // 10
    async with SessionLocal() as session:
        await session.execute(text("SET LOCAL enable_seqscan = off"))
        plan = await _explain(session, _labeled_stmt(session, _PLAN_DAY_WINDOWS))
    _assert_raw_branch_bounded(plan, expected_labeled, context=("labeled", _PLAN_W))
